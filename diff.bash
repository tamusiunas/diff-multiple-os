...
then
  echo ""
  echo "diff.bash version $VERSION"
  echo "It's an universal diff script that runs using Unix-like OS and Windows (with git-scm or WSL)"
  echo "It uses GNU diff distributed with Unix-like OS or bash provided by git-scm and WSL"
  echo "For Windows it requires git-scm <https://git-scm.com> or Windows WSL installed"
  echo "Usage: "
  echo "           \$ bash $0 \"<diff parameters>\""
  echo "Important: \"<diff parameters>\" $(tput bold)MUST$(tput sgr0) have double quotes."
  ...
  echo "For more information about diff parametes: "
  echo "           \$ man diff # or <https://man7.org/linux/man-pages/man1/diff.1.html>"
  echo ""
  exit 1
fi
